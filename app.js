...
function notificationList(limit = 20) {
  ...
  return `<div class="list">${items.map(n => `
    <div class="list-item ${["lesson_plan","personnel_profile","leave_request","substitute_batch","substitute_lesson"].includes(n.entity_type) ? "notification-clickable" : ""}"
...
function timetableStudentExportClassIds() {
  ...
  for (const c of state.timetableClasses) {
    if (stageRows.some(s => String(s.academic_year) === String(c.academic_year) && s.stage_code === c.stage_code)) {
      allowed.add(c.id);
    }
  }
  return [...allowed];
...
function timetableExportHtml() {
  ...
  return `${timetablePeriodFiltersHtml()}<div class="grid-2 timetable-export-grid">
    <section class="panel"><div class="panel-head"><div class="panel-title-wrap"><h3>ตารางเรียนของนักเรียน</h3><p>เลือกชั้น/ห้อง แล้ว Export ตามแบบโรงเรียน</p></div></div>
      ...
      <button class="btn btn-primary timetable-export-button" id="export-class-timetable" ${classes.length?"":"disabled"}>▤ Preview / Export PDF</button>
    </section>
    <section class="panel"><div class="panel-head"><div class="panel-title-wrap"><h3>ตารางสอนของครู</h3><p>${canViewAllTimetables()?"หัวหน้าวิชาการเลือกครูได้ทุกคน":"Export ได้เฉพาะตารางของคุณ"}</p></div></div>
      ...
      <button class="btn btn-primary timetable-export-button" id="export-teacher-timetable">▤ Preview / Export PDF</button>
    </section>
  </div>`;
}

...
async function loadPersonnelWorkspace() {
  ...
  if (recordsRes.error) {
    console.error("Personnel records load failed", recordsRes.error);
    state.personnelRecords = [];
  } else {
    state.personnelRecords = recordsRes.data || [];
  }
  state.personnelRoster = rosterRes.error ? [state.profile] : (rosterRes.data || []);
  state.personnelOwnRecord = state.personnelRecords.find(r => r.user_id === state.user.id) || null;
  ...
  if (!reviewer) state.selectedPersonnelUserId = state.user.id;
  if (state.selectedPersonnelUserId && !state.personnelRoster.some(p => p.id === state.selectedPersonnelUserId)) {
    state.selectedPersonnelUserId = reviewer ? null : state.user.id;
  }
}
...
function personnelWorkspaceHtml(module) {
  if (state.selectedPersonnelUserId) return personnelDetailHtml(state.selectedPersonnelUserId);
  if (isPersonnelReviewer()) return personnelDirectoryHtml();
  return personnelDetailHtml(state.user.id);
}
...
function printPersonnelA4(profile,record,assets) {
  ...
  const html = `<!doctype html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ข้อมูลบุคลากร - ${escapeHtml(personnelDisplayName(record,profile))}</title><style>${personnelA4Styles()}</style></head><body class="a4-print-body">${buildPersonnelA4Html(profile,record,assets)}<script>window.addEventListener('load',()=>{const imgs=[...document.images];Promise.all(imgs.map(img=>img.complete?Promise.resolve():new Promise(r=>{img.onload=r;img.onerror=r;}))).finally(()=>setTimeout(()=>window.print(),250));});<\/script></body></html>`;
...
function printLeaveReport(kind,personId,assets) {
  ...
  const html=`<!doctype html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>${leaveReportPrintStyles()}</style></head><body class="a4-print-body">${body}<script>window.addEventListener('load',()=>{const imgs=[...document.images];Promise.all(imgs.map(img=>img.complete?Promise.resolve():new Promise(r=>{img.onload=r;img.onerror=r;}))).finally(()=>setTimeout(()=>window.print(),250));});<\/script></body></html>`;
  try{w.opener=null;}catch{}
  w.document.open();w.document.write(html);w.document.close();
}
...
function printLeaveA4(request,assets) {
  const w=window.open("","_blank");
  if(!w) return toast("เปิดหน้าพิมพ์ไม่ได้","กรุณาอนุญาต Pop-up สำหรับเว็บไซต์นี้","error");
  const html=`<!doctype html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ใบลา #${request.request_no}</title><style>${leaveA4Styles()}</style></head><body class="a4-print-body">${buildLeaveA4Html(request,assets)}<script>window.addEventListener('load',()=>{const imgs=[...document.images];Promise.all(imgs.map(img=>img.complete?Promise.resolve():new Promise(r=>{img.onload=r;img.onerror=r;}))).finally(()=>setTimeout(()=>window.print(),250));});<\/script></body></html>`;
  try{w.opener=null;}catch{}
  w.document.open();w.document.write(html);w.document.close();
}
...
async function timetablePdfPreview(kind,id) {
  ...
  modal.querySelector("#tt-a4-print").addEventListener("click",()=>{
    ...
    w.document.write(`<!doctype html><html lang="th"><head><meta charset="UTF-8"><title>ตารางสอน</title><style>${timetableA4Styles()}</style></head><body class="a4-print-body">${body}<script>window.addEventListener('load',()=>setTimeout(()=>window.print(),250));<\/script></body></html>`);w.document.close();
  });
}
...
async function loadProjectWorkspace(){const [p,a,b]=await Promise.all([supabase.from("school_projects").select("*").order("academic_year",{ascending:false}).order("project_no"),supabase.from("project_activities").select("*").order("activity_no"),supabase.from("budget_disbursements").select("*").order("created_at",{ascending:false})]);state.schoolProjects=p.error?[]:(p.data||[]);state.projectActivities=a.error?[]:(a.data||[]);state.budgetDisbursements=b.error?[]:(b.data||[]);if(!state.projectAcademicYear)state.projectAcademicYear=currentAcademicPeriod().academicYear;}
...
function projectBudgetRequestsHtml(){const rows=state.budgetDisbursements.filter(b=>Number(b.academic_year)===projectCurrentYear());return `${projectYearFilterHtml()}<section class="panel"><div class="panel-head"><div class="panel-title-wrap"><h3>คำขอเบิกงบประมาณ</h3><p>เป็นระบบติดตามและจัดทำเอกสาร ไม่ใช่ Online Approval</p></div></div>${rows.length?`<div class="table-wrap"><table class="table"><thead><tr><th>โครงการ/กิจกรรม</th><th>ครั้งที่</th><th>วันที่</th><th>ผู้ขอ</th><th>รายละเอียด</th><th>จำนวน</th><th>สถานะ</th><th></th></tr></thead><tbody>${rows.map(b=>{const p=state.schoolProjects.find(x=>x.id===b.project_id),a=state.projectActivities.find(x=>x.id===b.activity_id);return `<tr><td><strong>${escapeHtml(a?.activity_no||p?.project_no||"—")}</strong><br><span class="table-muted">${escapeHtml(a?.activity_name||p?.project_name||"")}</span></td><td>${b.request_round}</td><td>${thaiDateOnly(b.request_date)}</td><td>${escapeHtml(b.requester_name)}</td><td>${escapeHtml(b.description)}</td><td>${money(b.amount)}</td><td>${budgetStatusPill(b.status)}</td><td><button class="btn btn-ghost" data-open-budget-request="${b.id}">รายละเอียด</button></td></tr>`}).join("")}</tbody></table></div>`:`<div class="empty"><strong>ยังไม่มีคำขอเบิก</strong></div>`}</section>`;}
...
function budgetRequestDetailHtml(r){const p=state.schoolProjects.find(x=>x.id===r.project_id),a=state.projectActivities.find(x=>x.id===r.activity_id),own=r.requester_id===state.user.id;return `<section class="project-detail"><div class="personnel-detail-top"><div><button class="type-back-link" id="budget-request-back">← กลับรายการเบิก</button><span class="eyebrow dark">ครั้งที่ ${r.request_round}</span></div><div class="personnel-detail-actions">${own&&r.status==="draft"?`<button class="btn btn-primary" id="prepare-budget-request">จัดทำเอกสาร</button>`:""}${["document_ready","printed","paid"].includes(r.status)?`<button class="btn btn-secondary" id="budget-request-pdf">▤ ตั้งค่าผู้ลงนาม / Export PDF</button>`:""}${isPlanBudgetHead()&&["document_ready","printed"].includes(r.status)?`<button class="btn btn-success" id="mark-budget-paid">บันทึกว่าเบิกจ่ายแล้ว</button>`:""}</div></div><section class="project-detail-hero"><div>${budgetStatusPill(r.status)}<h2>${escapeHtml(a?`${a.activity_no} ${a.activity_name}`:`${p?.project_no||""} ${p?.project_name||""}`)}</h2><p>ผู้ขอเบิก: ${escapeHtml(r.requester_name)} · ${thaiDateOnly(r.request_date)}</p></div><div class="project-big-budget"><strong>${money(r.amount)}</strong><span>บาท</span></div></section>${budgetLineItemsTableHtml(r)}<div class="detail-grid" style="margin-top:14px">${planField("โครงการ",`${p?.project_no||"—"} ${p?.project_name||""}`)}${planField("กิจกรรม",a?`${a.activity_no} ${a.activity_name}`:"เบิกจากโครงการหลัก")}${planField("ปีการศึกษา",r.academic_year)}${planField("ภาคเรียน",r.semester)}${planField("จำนวนเงินรวม",`${money(r.amount)} บาท`)}${planField("สถานะ",BUDGET_STATUS_LABEL[r.status]||r.status)}${planField("หมายเหตุ",r.notes||"—")}</div><div class="project-paper-note"><strong>ไม่มี Online Approval</strong><span>หัวหน้าฝ่ายและฝ่ายแผนงานฯ ใช้ระบบเพื่อตรวจดูข้อมูล ส่วนการรับรองจริงใช้เอกสารที่พิมพ์ออกมาเซ็นสด</span></div></section>`;}
function projectWorkspaceHtml(){const r=selectedBudgetRequest();if(r)return budgetRequestDetailHtml(r);const p=selectedProject();if(p){if(state.selectedProjectActivityId){const a=state.projectActivities.find(x=>x.id===state.selectedProjectActivityId);if(a)return activityDetailHtml(a);}return projectDetailHtml(p);}return `${projectHeroHtml()}${projectTabsHtml()}${state.projectView==="overview"?projectOverviewHtml():state.projectView==="budget"?projectBudgetRequestsHtml():projectMineHtml()}`;}
...
function signerFields(s={}){return [["requester","ผู้ขอเบิก"],["department_head","หัวหน้าฝ่ายเจ้าของโครงการ"],["plan_budget","ผู้รับผิดชอบแผนงานและงบประมาณ"],["director","ผู้อำนวยการโรงเรียน"],["inspector1","กรรมการตรวจรับคนที่ 1"],["inspector2","กรรมการตรวจรับคนที่ 2"],["inspector3","กรรมการตรวจรับคนที่ 3"]].map(([k,l])=>`<div class="budget-signer-block"><strong>${l}</strong><div class="form-row"><div class="field"><label>ชื่อ-นามสกุล</label><input class="input" data-signer-name="${k}" value="${escapeHtml(s?.[k]?.name||"")}"></div><div class="field"><label>ตำแหน่ง</label><input class="input" data-signer-position="${k}" value="${escapeHtml(s?.[k]?.position||"")}"></div></div></div>`).join("");}
function budgetSignersModal(r){const m=document.createElement("div");m.className="modal-backdrop";m.innerHTML=`<div class="modal modal-wide"><div class="modal-head"><div><h3>ตั้งค่าผู้ลงนามก่อน Export</h3><p>กรอกชื่อและตำแหน่งเอง แล้วเว้นเส้นสำหรับเซ็นสดใน PDF</p></div><button class="modal-close">×</button></div><div class="budget-signer-form">${signerFields(r.signers||{})}</div><div class="modal-actions"><button class="btn btn-ghost modal-cancel">ยกเลิก</button><button class="btn btn-primary" id="save-signers">บันทึกและ Preview PDF</button></div></div>`;document.body.appendChild(m);const close=()=>m.remove();m.querySelector(".modal-close").onclick=close;m.querySelector(".modal-cancel").onclick=close;m.querySelector("#save-signers").onclick=async e=>{const s={};m.querySelectorAll("[data-signer-name]").forEach(i=>{const k=i.dataset.signerName;s[k]={name:i.value.trim(),position:m.querySelector(`[data-signer-position="${k}"]`)?.value.trim()||""};});buttonLoading(e.target,true);const {error}=await supabase.from("budget_disbursements").update({signers:s,status:r.status==="document_ready"?"printed":r.status}).eq("id",r.id);buttonLoading(e.target,false);if(error)return toast("บันทึกผู้ลงนามไม่สำเร็จ",error.message,"error");close();await loadProjectWorkspace();await budgetRequestPdfPreview(state.budgetDisbursements.find(x=>x.id===r.id));};}
...
function budgetPdfStyles(){return `${projectPdfStyles()} .budget-line-pdf-table{width:100%;border-collapse:collapse;font-size:10.8pt;margin-top:4mm}.budget-line-pdf-table th,.budget-line-pdf-table td{border:1px solid #222;padding:1.6mm 2mm;vertical-align:top}.budget-line-pdf-table th{background:#fafafa;text-align:center}.budget-line-pdf-table .no{width:10mm;text-align:center}.budget-line-pdf-table .amount{width:38mm;text-align:right}.budget-line-pdf-table .total td{font-weight:700;background:#fafafa}.budget-doc-note{font-size:10.5pt;margin-top:2.5mm}.budget-doc-sign-grid{display:grid;grid-template-columns:1fr 1fr;gap:7mm;margin-top:6mm}.budget-doc-inspector-title{text-align:center;font-size:12pt;font-weight:700;margin-top:5mm;margin-bottom:1mm}.budget-doc-inspectors{display:grid;grid-template-columns:repeat(3,1fr);gap:4mm;margin-top:2mm}.budget-doc-sign{text-align:center;font-size:10.5pt;min-height:29mm}.budget-doc-sign .line{margin-top:9mm}`;}
function docSign(title,s={}){return `<div class="budget-doc-sign"><div class="line">ลงชื่อ............................................</div><div>(${escapeHtml(s.name||"................................")})</div><strong>${escapeHtml(s.position||title)}</strong></div>`;}
async function budgetRequestPdfPreview(r){const p=state.schoolProjects.find(x=>x.id===r.project_id),a=state.projectActivities.find(x=>x.id===r.activity_id),s=r.signers||{},items=budgetLineItems(r),logo=await schoolLogoDataUrl(),body=`<article class="a4-document"><div class="a4-document-inner"><header class="project-doc-head">${logo?`<img class="a4-school-logo" src="${logo}">`:""}<h1>${escapeHtml(schoolName())}</h1><div class="project-doc-meta">${escapeHtml(schoolAddress())}<br>${escapeHtml(educationOffice())}</div><h2>ใบขอเบิกใช้งบประมาณโครงการ / กิจกรรม</h2></header><div class="project-doc-info"><div><strong>โครงการ:</strong> ${escapeHtml(p?.project_no||"—")} ${escapeHtml(p?.project_name||"")}</div><div><strong>กิจกรรม:</strong> ${escapeHtml(a?`${a.activity_no} ${a.activity_name}`:"เบิกจากโครงการหลัก")}</div><div><strong>เบิกครั้งที่:</strong> ${r.request_round}</div><div><strong>วันที่:</strong> ${thaiDateOnly(r.request_date)}</div><div><strong>ปีการศึกษา:</strong> ${r.academic_year}</div><div><strong>ภาคเรียน:</strong> ${r.semester}</div></div><table class="budget-line-pdf-table"><thead><tr><th class="no">#</th><th>รายการใช้เงิน</th><th class="amount">จำนวนเงิน (บาท)</th></tr></thead><tbody>${items.map((item,i)=>`<tr><td class="no">${i+1}</td><td>${escapeHtml(item.description)}</td><td class="amount">${money(item.amount)}</td></tr>`).join("")}<tr class="total"><td colspan="2"><strong>รวมทั้งสิ้น</strong></td><td class="amount"><strong>${money(r.amount)}</strong></td></tr></tbody></table><div class="budget-doc-note"><strong>หมายเหตุ:</strong> ${escapeHtml(r.notes||"—")}</div><div class="budget-doc-sign-grid">${docSign("ผู้ขอเบิก",s.requester)}${docSign("หัวหน้าฝ่ายเจ้าของโครงการ",s.department_head)}${docSign("ผู้รับผิดชอบแผนงานและงบประมาณ",s.plan_budget)}${docSign("ผู้อำนวยการโรงเรียน",s.director)}</div><div class="budget-doc-inspector-title">คณะกรรมการตรวจรับ</div><div class="budget-doc-inspectors">${docSign("กรรมการตรวจรับคนที่ 1",s.inspector1)}${docSign("กรรมการตรวจรับคนที่ 2",s.inspector2)}${docSign("กรรมการตรวจรับคนที่ 3",s.inspector3)}</div></div></article>`;openPrintPreview(body,budgetPdfStyles(),"ใบขอเบิกงบประมาณ");}
...
function homeVisitPhotoSlot(r,kind,label){const p=homeVisitPhotosFor(r.id).find(x=>x.kind===kind);return `<div class="hv-photo-slot"><div class="hv-photo-preview" data-hv-photo-preview="${kind}">${p?`<span>✓ มีรูปแล้ว</span>`:`<span>ยังไม่มีรูป</span>`}</div><strong>${escapeHtml(label)}</strong><input type="file" accept="image/png,image/jpeg,image/webp" data-hv-photo-file="${kind}"><button class="btn btn-secondary" type="button" data-upload-hv-photo="${kind}">อัปโหลด / เปลี่ยนรูป</button></div>`}
...
async function saveHomeVisitStep(r){if(state.homeVisitStep===2)return saveHouseholdMembers(r);if([5,6].includes(state.homeVisitStep))return true;const values=formValuesObject(document.querySelector("#home-visit-form"));const f={...(r.form_data||{})};for(const [k,v] of Object.entries(values))f[k]=v;for(const name of ["flags","dependency","floor_material","wall_material","roof_material","water_source","electric_source","vehicles","household_goods"]){const els=[...document.querySelectorAll(`[name="${name}"]:checked`)];if(els.length||document.querySelector(`[name="${name}"]`))f[name]=els.map(x=>x.value)}const {error}=await supabase.from("home_visit_records").update({form_data:f}).eq("id",r.id);if(error){toast("บันทึกไม่สำเร็จ",error.message,"error");return false}toast("บันทึกแล้ว","ข้อมูลขั้นตอนนี้ถูกเก็บเป็นแบบร่างแล้ว","success");await loadHomeVisitWorkspace();return true}
...
async function uploadHomeVisitPhoto(r,kind){const input=document.querySelector(`[data-hv-photo-file="${kind}"]`),file=input?.files?.[0];if(!file)return toast("กรุณาเลือกรูปภาพ","PNG, JPG หรือ WebP ไม่เกิน 8 MB","error");if(file.size>8*1024*1024)return toast("ไฟล์ใหญ่เกินไป","รูปต้องไม่เกิน 8 MB","error");const ext=file.type==="image/png"?"png":file.type==="image/webp"?"webp":"jpg",path=`${r.id}/photos/${kind}-${crypto.randomUUID()}.${ext}`,old=homeVisitPhotosFor(r.id).find(x=>x.kind===kind);const up=await supabase.storage.from("home-visits").upload(path,file,{contentType:file.type,upsert:false});if(up.error)return toast("อัปโหลดรูปไม่สำเร็จ",up.error.message,"error");if(old){await supabase.from("home_visit_photos").delete().eq("id",old.id);await supabase.storage.from("home-visits").remove([old.storage_path])}const ins=await supabase.from("home_visit_photos").insert({record_id:r.id,kind,storage_path:path,uploaded_by:state.user.id});if(ins.error){await supabase.storage.from("home-visits").remove([path]);return toast("บันทึกรูปไม่สำเร็จ",ins.error.message,"error")}toast("อัปโหลดรูปแล้ว","รูปจะถูกใช้ใน PDF อัตโนมัติ","success");await renderDashboard()}
...
async function homeVisitRecordAssets(r){const photos=homeVisitPhotosFor(r.id),sigs=homeVisitSignaturesFor(r.id),photoData={},sigData={};for(const p of photos)photoData[p.kind]=await homeVisitAssetData(p.storage_path,p.bucket_id);for(const s of sigs){if(s.method==="stored"&&s.signature_id){const master=state.signatures.find(x=>x.id===s.signature_id);if(master)sigData[s.signer_type]=await homeVisitAssetData(master.storage_path,master.bucket_id||"signatures")}else if(s.storage_path)sigData[s.signer_type]=await homeVisitAssetData(s.storage_path,s.bucket_id||"home-visits")}return{photos:photoData,sigs:sigData}}
function yes(v){return v?"✓":"□"}
function hvPdfHeader(r,logo,title="แบบบันทึกการเยี่ยมบ้านนักเรียน"){return `<header class="hv-pdf-head">${logo?`<img src="${logo}" class="a4-school-logo">`:""}<div><h1>${escapeHtml(schoolName())}</h1><p>${escapeHtml(schoolAddress())} · ${escapeHtml(educationOffice())}</p><h2>${escapeHtml(title)}</h2><small>ปีการศึกษา ${escapeHtml(r.academic_year)} ภาคเรียนที่ ${r.semester} · ${escapeHtml(r.class_label)} · ครูประจำชั้น ${escapeHtml(r.homeroom_teacher_name||"—")} · ผู้บันทึก ${escapeHtml(r.recorder_name)}</small></div></header>`}
...
function a4DocumentStyles() {
  return `
    @font-face{font-family:"THSarabunDocument";src:local("TH Sarabun PSK"),local("THSarabunPSK"),local("TH Sarabun New");font-style:normal;font-weight:400;}
    @font-face{font-family:"THSarabunDocument";src:local("TH Sarabun PSK Bold"),local("THSarabunPSK Bold"),local("TH Sarabun New Bold"),local("TH Sarabun PSK"),local("TH Sarabun New");font-style:normal;font-weight:700;}
    @page{size:A4;margin:14mm 16mm 16mm;}
    *{box-sizing:border-box;}
    body.a4-print-body{margin:0;background:#eef2f7;color:#111;font-family:"THSarabunDocument","Sarabun",sans-serif;}
    .a4-document{width:210mm;min-height:297mm;margin:0 auto;background:white;padding:16mm 18mm 18mm;font-family:"THSarabunDocument","Sarabun",sans-serif;font-size:16pt;line-height:1.18;color:#111;}
...
function printLessonA4(detail, assets) {
  const w = window.open("", "_blank");
  if (!w) return toast("เปิดหน้าพิมพ์ไม่ได้", "กรุณาอนุญาต Pop-up สำหรับเว็บไซต์นี้", "error");
  const html = `<!doctype html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>แผนการสอน - ${escapeHtml(detail.plan.subject_name || "เอกสาร")}</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;700&display=swap" rel="stylesheet"><style>${a4DocumentStyles()}</style></head><body class="a4-print-body">${buildA4DocumentHtml(detail, assets)}<script>window.addEventListener('load',()=>{const imgs=[...document.images];Promise.all(imgs.map(img=>img.complete?Promise.resolve():new Promise(r=>{img.onload=r;img.onerror=r;}))).finally(()=>setTimeout(()=>window.print(),250));});<\/script></body></html>`;
  try { w.opener = null; } catch {}
  w.document.open();
...
function bindDashboardEvents() {
  document.querySelectorAll("[data-view]").forEach(el => {
    el.addEventListener("click", () => {
      ...
      if (nextView === "module:personnel_records" && state.currentView !== "module:personnel_records") {
        state.selectedPersonnelUserId = isPersonnelReviewer() ? null : state.user.id;
      }
      ...
      if (nextView === "module:project_management" && state.currentView !== "module:project_management") {
        ...
        state.projectView=isPlanBudgetHead()?"overview":"mine"; state.projectDepartmentFilter="academic";
      }
  ...
  document.querySelectorAll("[data-personnel-user]").forEach(btn => btn.addEventListener("click", () => {
    state.selectedPersonnelUserId = btn.dataset.personnelUser;
    renderDashboard();
  }));
  document.querySelector("#personnel-back")?.addEventListener("click", () => {
    state.selectedPersonnelUserId = null;
    renderDashboard();
  });
  document.querySelector("#personnel-my-record")?.addEventListener("click", () => {
    state.selectedPersonnelUserId = state.user.id;
    renderDashboard();
  });
  ...
  document.querySelector("#budget-request-pdf")?.addEventListener("click",()=>{const r=selectedBudgetRequest();if(r)budgetSignersModal(r);});
